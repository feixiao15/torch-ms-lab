# -*- coding: utf-8 -*-
"""
PyTorch vs MindSpore AlexNet 对比实验：
1) pth 转 ckpt
2) ImageNet 输入（默认 500 张），logits 误差 + 预测一致率 + Top-1 准确度

用法: python alexnet/compare_torch_ms.py [--num-samples N] [--skip-convert] [--imagenet-dir DIR]
"""
import os
import sys
import argparse
...
def _imagenet_paths_sorted_by_index(imagenet_dir, num_samples):
    ...
    return sorted(paths, key=_path_to_val_index)[:num_samples]
...
def get_imagenet_samples(num_samples, imagenet_dir=None):
    """ImageNet 图片目录，Resize 224 + CenterCrop + ImageNet 归一化。返回 (samples, paths)。"""
    ...
    tf = T.Compose([
        ...
        T.ToTensor(),
        T.Normalize(IMAGENET_MEAN.ravel(), IMAGENET_STD.ravel()),
    ])
    samples = []
    for p in paths:
...
def main():
    ...
    pt_net = alexnet(weights=AlexNet_Weights.IMAGENET1K_V1).to(device).eval()
    if os.path.isfile(PTH):
        raw = torch.load(PTH, map_location=device, weights_only=True)
        pt_net.load_state_dict(raw, strict=True)
