...
def convert_pth_to_ckpt():
    from mobilenet.pth_to_ckpt import convert
    convert()
...
def get_labels_for_paths(gt_file, paths, meta_mat_path=None):
    """标签第 i 行(0-based)=图像 0000000(i+1).JPEG，故 line_idx = val_index - 1。"""
...
def get_imagenet_samples(num_samples, imagenet_dir=None):
    """ImageNet 图片，Resize 256 + CenterCrop 224 + ImageNet 归一化。返回 (samples, paths)。"""
    ...
    # MobileNetV2 官方预处理：Resize 256 -> CenterCrop 224 -> Normalize
    tf = T.Compose([
        T.Resize(256),
...
def main():
    ...
    parser.add_argument("--meta-mat", type=str, default=META_MAT, help="meta.mat 路径")
    ...
    pt_net = mobilenet_v2(weights=MobileNet_V2_Weights.IMAGENET1K_V1).to(device).eval()
